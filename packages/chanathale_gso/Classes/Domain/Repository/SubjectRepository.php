...
class SubjectRepository extends \TYPO3\CMS\Extbase\Persistence\Repository {
    ...
    public function _findAllByFrontendUser (int $frontUserUid) : ?QueryResultInterface {
        ...
        $queryBuilder = GeneralUtility::makeInstance(ConnectionPool::class)->getConnectionForTable('tx_chanathalegso_domain_model_subject')->createQueryBuilder();
        $uids = $queryBuilder->select('uid')
            ->from('tx_chanathalegso_domain_model_subject')
            ->where(
